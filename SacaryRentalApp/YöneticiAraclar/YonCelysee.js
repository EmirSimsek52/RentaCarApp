...
export default function YonCelysee({navigation}){
  const [data, setData] = useState([])

 
  const readPosts = async () => {
    const RezarvasyonCollection = await firestore().collection('Celysee').get()
                    ...
                    data.map(item => {
                        return(
                                <View  style={{width:300,alignSelf:'center'}} key={item.id}>
                                    ...
                                    <TouchableOpacity  onPress={() => {navigation.navigate('GnCelysee', {
