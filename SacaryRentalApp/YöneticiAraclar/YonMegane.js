...
export default function YonMegane({navigation}){
  const [data, setData] = useState([])

 
  const readPosts = async () => {
    const RezarvasyonCollection = await firestore().collection('Megane').get()
                    ...
                    data.map(item => {
                        return(
                                <View  style={{width:300,alignSelf:'center'}} key={item.id}>
                                    ...
                                    <TouchableOpacity  onPress={() => {navigation.navigate('GnMegane', {
