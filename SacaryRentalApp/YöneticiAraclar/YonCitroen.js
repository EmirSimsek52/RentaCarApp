...
export default function YonCitroen({navigation}){
  const [data, setData] = useState([])

 
  const readPosts = async () => {
    const RezarvasyonCollection = await firestore().collection('Citroen').get()
                    ...
                    data.map(item => {
                        return(
                                <View  style={{width:300,alignSelf:'center'}} key={item.id}>
                                    ...
                                    <TouchableOpacity  onPress={() => {navigation.navigate('GnCitroen', {
