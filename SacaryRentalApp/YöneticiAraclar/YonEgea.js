...
export default function YonEgea({navigation}){
  const [data, setData] = useState([])

 
  const readPosts = async () => {
    const RezarvasyonCollection = await firestore().collection('Egea').get()
                    ...
                    data.map(item => {
                        return(
                                <View  style={{width:300,alignSelf:'center'}} key={item.id}>
                                    ...
                                    <TouchableOpacity  onPress={() => {navigation.navigate('GnEgea', {
